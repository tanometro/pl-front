...
const DashboardSummary: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h2 className="text-2xl font-bold text-center mb-6">Resumen de Prestamos</h2>

      {/* Gráficos de resumen de préstamos y deuda */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Gráfico de Préstamos (Propios vs. De Terceros) */}
        <div className="bg-white shadow-md rounded-lg p-6 flex flex-col items-center">
          <h3 className="text-lg font-semibold mb-4">Prestamos</h3>
          <Doughnut data={loansData} />
          <p className="mt-4 text-sm text-gray-500">Propios vs Terceros</p>
        </div>

        {/* Gráfico de Deuda Acumulada */}
        <div className="bg-white shadow-md rounded-lg p-6 flex flex-col items-center">
          <h3 className="text-lg font-semibold mb-4">Deuda Total</h3>
          <FaDollarSign className="text-4xl text-red-500 mb-2" />
          <Bar
            data={debtData}
            options={{ indexAxis: "y", scales: { x: { beginAtZero: true } } }}
          />
          <p className="mt-4 text-sm text-gray-500">Total de deuda acumulada</p>
        </div>
